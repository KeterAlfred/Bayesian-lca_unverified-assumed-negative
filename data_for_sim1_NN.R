...
sim1_dat <- foreach(m=c(1:M)+j, .inorder = FALSE) %do% {
  ...
  pattern1 <- aggregate( ones ~   Y1 + Y2  + Y4 + Y50 + Y60, data = dat.tested, function(x)NROW(x))
  ...
  jags.sim.data_NN = list( N = sum(jags.data[,"Freqobs"]),
  ...
  names(jags.sim.data_NN) <- c("N","Freqobs","y","J","K","D","dim_D")
  
  
  
  
  return( jags.sim.data_NN )
}
...
# saveRDS(sim1_dat, file = "C:\\Users\\Keter\\OneDrive - ITG\\EDCTP\\PhD\\lca\\Vukuzazi\\revised analysis\\vb_sim_analysis\\sim1_dat.RData")
